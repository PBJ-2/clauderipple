...
**업데이트.** 새 버전을 같은 방법으로 설치하거나(zip이면 아무 데나 풀고) 앱을 열면 됩니다. 0.1.2부터는 앱이
"아직 떠 있는 라우터가 이전 버전"임을 알아채고 알림과 함께 새 버전으로 바꿉니다. 0.1.2 전에는 그런 장치가 없어서
0.1.0/0.1.1 설치는 라우터를 재시작하기 전까지 옛 코드로 계속 답합니다. 게다가 그 버전들의 트레이 →
**라우터 재시작**은 Windows에서 실제로는 재시작하지 못했습니다. 0.1.2를 설치한 뒤 한 번만 로그아웃 후 다시
로그인(또는 재부팅)하십시오. 그다음부터는 앱이 알아서 합니다.
...
라우터. Node 설치는 필요 없습니다. 그다음 트레이 아이콘 → **브라우저에서 대시보드 열기…** → **프로바이더** → ChatGPT 추가
