...
async function poll() {
    try {
        ...
        downNotified = false;
        if (winOffline)
            showWindowContent(); // the router answered: swap the notice for the GUI
    }
    catch (e) {
        last = null;
        lastError = e.message;
...
function icon(state) {
    const file = node_path_1.default.join(__dirname, "..", "assets", state === "ok" ? "trayTemplate.png" : state === "warn" ? "trayWarnTemplate.png" : "trayDownTemplate.png");
    const img = electron_1.nativeImage.createFromPath(file);
 ...
 */
function showWindowContent(opts = {}) {
    if (!win || win.isDestroyed())
        return;
    if (last) {
        winOffline = false;
...
function offlineNotice(starting) {
    const esc = (s) => s.replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
    const html = `<!doctype html><html lang="${electron_1.app.getLocale().startsWith("ko") ? "ko" : "en"}"><meta charset="utf-8">
...
function packagedRuntime() {
    ...
    if (!bundle)
        return null;
    const resources = node_path_1.default.join(bundle, "Contents", "Resources", "clauderipple");
    return {
        node: process.execPath,
        env: { ELECTRON_RUN_AS_NODE: "1" },
        cli: node_path_1.default.join(resources, "packages", "cli", "src", "index.ts"),
    };
}
...
function cliPaths() {
    ...
    return { node: process.execPath, env: { ELECTRON_RUN_AS_NODE: "1" }, cli: node_path_1.default.resolve(__dirname, "..", "..", "cli", "src", "index.ts") };
}
...
async function setup() {
    ...
    await electron_1.dialog.showMessageBox({ message: L.setupDone, detail: out });
    void poll();
}
...
function render() {
    ...
    const template = [
        ...
        ...(detail ? [{ label: detail, click: openWindow }] : []),
        { type: "separator" },
...
electron_1.app.whenReady().then(() => {
    L = STRINGS[electron_1.app.getLocale().startsWith("ko") ? "ko" : "en"];
    if (process.platform === "darwin")
        electron_1.app.dock?.hide();
    ...
    if (needsSetup())
        void promptForSetup();
    if (process.env.CLAUDERIPPLE_OPEN_WINDOW)
        openWindow(); // dev/testing: show the GUI window immediately
});
