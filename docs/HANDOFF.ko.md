...
**1. 라이브 라우터는 오늘 코드가 반영돼 있다.** 09:32:52에 드레인 재시작했고, 스키마 수정 뒤 한 번 더
재시작했다. 어제 웹검색 가드(`1f4202e`)도 이때 같이 올라갔다. 인플라이트 유실 없음.
...
**1. 모델 자동 라우팅** — 어제 1번 그대로다. 주군이 하겠다고 하셨고 아직 손대지 않았다.
`resolve()`가 `cfg.direct`와 `cfg.routes`만 보고, 프로바이더의 `models` 목록을 라우팅에 안 쓴다.

**2. 이슈 #7 (재현 대기).** 코드는 안 고쳤다. 고친다면 정규화가 아니라 **키 고정**이다 — 깨끗한
씨앗을 기억해두고 나중 씨앗이 `U+FFFD` 덩어리만 다르면 기억한 **키**를 재사용한다. 본문은 그대로
흘려보내므로 바이트 전달 원칙이 산다. 독립 재현이 나오기 전에는 하지 마라.

**3. ChatGPT 웹검색 백엔드 (쿼터 대기).** 어제 3번 그대로.

**4. 무자격증명 검색 백엔드.** 어제 4번 그대로.

**5. `docs/OPENCODEX.md` §7에 `claude gateway` 한 줄.** 위 "새로 확인된 사실"의 요지를 옮기면 된다.
